...
namespace CSimple.Pages
{
    public partial class OrientPage : ContentPage, IDrawable
    {
        ...
        public OrientPage(OrientPageViewModel viewModel)
        {
            ...
            _viewModel.Nodes.CollectionChanged += (s, e) =>
            {
                NodeCanvas?.Invalidate();

                // Subscribe to property changes on new nodes for ExecutionState updates
                if (e.NewItems != null)
                {
                ...
                // Unsubscribe from property changes on removed nodes
                if (e.OldItems != null)
                {
            ...
            // Subscribe to property changes on existing nodes
        ...
        public void Draw(ICanvas canvas, RectF dirtyRect)
        {
            ...
            foreach (var node in _viewModel.Nodes)
            {
                ...
                // --- MODIFIED Border with Execution State Support ---
                ...
                // Set border color based on execution state (if running) or selection state
                // Priority: 1. Execution state (if not Pending), 2. Selection state, 3. Default
                Color borderColor;
                if (node.ExecutionState != ViewModels.ExecutionState.Pending)
                {
                    // Use execution state color for visual feedback during pipeline execution
                    borderColor = node.ExecutionBorderColor;
                }
                else if (node.IsSelected)
                {
                    // Use selected color when node is selected but not executing
                    borderColor = selectedStrokeColor;
                }
                else
                {
                    // Use default node border color
                    borderColor = nodeStrokeColor;
                }

                canvas.StrokeColor = borderColor;
                // --- END MODIFIED Border with Execution State Support ---
        ...
        private void DrawLegend(ICanvas canvas, RectF dirtyRect, Color backgroundColor, Color textColor)
        {
            ...
            // TODO: Implement actual legend drawing if needed
        ...
        /// <summary>
        /// Handles property changes on individual nodes to trigger canvas invalidation for ExecutionState changes
        /// </summary>
        private void OnNodePropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            // Invalidate canvas when ExecutionState or ExecutionBorderColor changes for visual feedback
            if (e.PropertyName == nameof(NodeViewModel.ExecutionState) ||
                e.PropertyName == nameof(NodeViewModel.ExecutionBorderColor))
            {
                NodeCanvas?.Invalidate();
            }
        }
